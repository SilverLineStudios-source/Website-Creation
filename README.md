...
from the 4534x4344 original. The first work tile plays a Vimeo reel in a lightbox; its numeric id
lives in one place, the `data-vimeo` attribute on that tile, and the id currently in the file is
Vimeo's placeholder `123456789` and must be swapped for the real one. Nothing is requested from
Vimeo until a visitor presses play. The other five tiles are still placeholders.
